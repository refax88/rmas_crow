    ...
    protected void think(int time, ChangeSet changed, Collection<Command> heard) {
        ...
        for (Human next : getTargets()) {
            if (next.getPosition().equals(location().getID())) {
                ...
                List<EntityID> path = search.search(me().getPosition(), next.getPosition(), connectivityGraph, distanceMatrix);
                if (path != null) {
        ...
        // Nothing to do
        List<EntityID> path = search.search(me().getPosition(), unexploredBuildings, connectivityGraph, distanceMatrix);
        if (path != null) {
