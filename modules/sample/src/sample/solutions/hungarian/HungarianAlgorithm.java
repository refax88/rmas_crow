 ...
 *
 * History:
 * The Hungarian Method based on ideas of Koenig and Egervary, and
...
/**
 * This is the basic assignment algorithm known as Hungarian Method. Where a
 * matching between the sets S and T is represented as a m x n matrix, with
 * |S|=m and |T|=n.
 * 
...
public class HungarianAlgorithm implements AssignmentAlgorithm {
	...
	public int[][] originalComputeAssignments(double[][] matrix) {
		...
		// "Do not process matrices where cols > rows!";
		...
		for (int i = 0; i < matrix[0].length; i++) {
			...
			// else could happen if we covered an invalid col, because no valid
			// entries
	...
	/**
	 * @param b
	 *            true if you want to cover the specified row; false to uncover
	 */
	...
	/**
	 * @param b
	 *            true if you want to cover the specified columns; false to
	 *            uncover
	 */
	private void coverColumn(int column, boolean b) {
		...
		if (b) {
			...
			assert !initialCovCols[column] : "We should uncover an invalid col:"
			        + column;
	...
	private void initStars(double costMatrix[][]) {
		...
		for (int row = 0; row < costMatrix.length; row++) {
			for (int col = 0; col < costMatrix[row].length; col++) {
				if (0 == costMatrix[row][col] && !rowHasStarredZero[row]
	...
	private void coverColumnsOfStarredZeroes() {
		for (int col = 0; col < starsByCol.length; col++) {
			...
			assert !(coveredCols[col] && starsByCol[col] != -1) : "We shouldn't have covered a starred column; col:"
			        + col;
