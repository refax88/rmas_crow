...
public final class LaunchComponents {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            String globalMemoryClass = "";
            try {
                globalMemoryClass = config.getValue("global_memory_class");    
            }
            catch (NoSuchConfigOptionException e) { }   // leave globalMemoryClass empty
            
            GlobalMemory globalMemory = null;
            if(!globalMemoryClass.isEmpty()) {
                globalMemory = instantiate(globalMemoryClass, GlobalMemory.class);
                if(globalMemory == null) {
                    Logger.error("Failed to instantiate global memory of class: " + globalMemoryClass);
                    System.exit(1);
                }
            }
            ...
            for (String next : toLaunch) {
                connect(launcher, next, gui, globalMemory);
            }
        }
        catch (IOException e) {
    ...
    private static void connect(ComponentLauncher launcher, String argLine, boolean gui, GlobalMemory globalMemory) throws InterruptedException, ConnectionException {
        ...
        for (int i = 0; i < count; ++i) {
            ...
            if(globalMemory != null && c instanceof GlobalMemoryAccess) {
                ((GlobalMemoryAccess) c).setGlobalMemory(globalMemory);
            }
