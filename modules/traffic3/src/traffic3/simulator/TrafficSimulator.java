...
import sample.BreadthFirstSearch;
import sample.DistanceInterface;
import sample.Graph;
import sample.SearchAlgorithm;
import sample.SearchBenchmark;
...
public class TrafficSimulator extends StandardSimulator implements GUIComponent {
    ...
    protected void processCommands(KSCommands c, ChangeSet changes) {
        ...
        System.out.println("COMMANDS:  "+ c.toString());
                //////
        //CODICE CHE AGGIUNGO ORA

        // AGGIUNTO ORA
        int time = c.getTime(); //
        for (TrafficAgent agent : manager.getAgents()) {
            Human human = agent.getHuman();
            if (human.getStandardURN().equals(StandardEntityURN.CIVILIAN)) {
                Civilian me = (Civilian) human;
                ...
                if (hp <= 0 || hp < consciousThreshold) {
                    ...
                    //sendRest(time);
                    send(new AKRest(human.getID(), time));
                    return;
                }
                if (damage > 0 && random.nextDouble() < ouchProbability) {
                    ...
                    try {
                        ...
                        send(new AKSay(human.getID(), time, message.getBytes("UTF-8")));
                        // }
                ...
                if (damage == 0 && buriedness == 0) {
                    System.out.println("MUOOOOOVITIIIIIIIIII");
                    System.out.println(human.getID());
                    System.out.println(time);
                    System.out.println(randomWalk(human).toString());
                    System.out.println("MUOOOOOVITIIIIIIIIII");
                    
                    //AKMove akmove = new AKMove(human.getID(), time, randomWalk(human));
                      //  this.handleMove(akmove);
                   // send(new AKMove(human.getID(), time, randomWalk(human)));
                    
                    
                    // Run for the refuge
                    //sendMove(time, new LinkedList<EntityID>());
                    // QUI SI MUOVONO
                    
                    List<EntityID> path = this.search(human.getPosition(), refugeIDs);
                    
                   // List<EntityID> path = search.search(human.getPosition(), refugeIDs, null, distanceMatrix);
                   // List<EntityID> path = search.search(human.getPosition(), refugeIDs, graph, distanceMatrix);
                    if (path != null) {
                        System.out.println(path.toString());
                      AKMove akmove = new AKMove(human.getID(), time, path);
                        this.handleMove(akmove);
                ...
                System.out.println("ID CIVILE!!! " + human.getID());
            }
        }
    ...
    //AGGIUNTO
        private List<EntityID> randomWalk(Human me) {
        ...
        for (int i = 0; i < RANDOM_WALK_LENGTH; ++i) {
            ...
            List<EntityID> possible = new ArrayList<EntityID>(this.getNeighbors(current));
        ...
        private void createGraph(StandardWorldModel world)
    {
        //resetAccessCount();
        ...
        for (Entity next : world)
        {
            ...
            private Set<EntityID> getNeighbors(EntityID id)
    {
    ...
    public List<EntityID> search(EntityID start, Collection<EntityID> goals)
    {
        ...
        do
        {
            ...
            Collection<EntityID> neighbours = this.getNeighbors(next);
