...
import traffic3.simulator.CivInfo;
...
public class TrafficAgent {
    ...
    */
    ...
    public void step(double dt, CivInfo civInfo) {
        if (mobile) {
            updateWalls(dt);
            updateGoals();
            computeForces(dt);
            updatePosition(dt, civInfo);
        }
    }
    ...
    */
    ...
    private void updatePosition(double dt, CivInfo civInfo) {

            double newVX = civInfo.getVX();//velocity[0] + dt * force[0];
            double newVY = civInfo.getVY();//velocity[1] + dt * force[1];
        ...
        double x = civInfo.getX();//location[0] + dt * newVX;
        double y = civInfo.getY();//location[1] + dt * newVY;
