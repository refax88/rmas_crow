...
public class DecentralizedAssignmentSimulator implements AssignmentInterface
{
    ...
    public Assignment compute(UtilityMatrix utility)
    {
        ...
        int MAX_ITERATIONS = 50; // TODO add to config file
        int byteMessage = 0;
        while (!done && iterations < MAX_ITERATIONS)
        {
            ...
            {   
            	...
            	Collection<AbstractMessage> messages = agent.sendMessages();
                
                //collect the byte size of the messages exchanged between agents
                for (AbstractMessage message : messages) {
                    byteMessage += SizeOf.deepSizeOf(message);
                }

            	if (messages != null) {
            		_com.send(agent.getAgentID(), agent.sendMessages());
            	}
            }
    ...
    private void initializeAgents(UtilityMatrix utilityM)
    {
        ...
        try
        {
            ...
            for (EntityID agentID : utilityM.getAgents()) 
            {
                ...
                UtilityMatrix localUM = utilityM;//convertToLocal(agentID, utilityM);
